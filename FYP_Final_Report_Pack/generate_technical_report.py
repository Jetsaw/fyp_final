...
FIGURE_DIR = PACK / "05_figures_technical"
SOURCE_FIGURE_DIR = PACK / "05_figures"
REPORT_DOCX = DRAFT_DIR / "FYP_FINAL_REPORT_TECHNICAL.docx"
REPORT_DOCX_FALLBACK = DRAFT_DIR / "FYP_FINAL_REPORT_TECHNICAL_UI_AVATAR.docx"
REPORT_MD = DRAFT_DIR / "FYP_FINAL_REPORT_TECHNICAL.md"
SUMMARY_JSON = PACK / "TECHNICAL_REPORT_BUILD_SUMMARY.json"
...
REFERENCE_LIST = [
    ...
    (
        "Unsloth, Qwen3 fine-tuning and Unsloth training documentation.",
        "https://unsloth.ai/docs/models/tutorials/qwen3-how-to-run-and-fine-tune",
    ),
    (
        "UnslothAI GitHub repository.",
        "https://github.com/unslothai/unsloth",
    ),
    (
        "Meta FAISS GitHub repository.",
        "https://github.com/facebookresearch/faiss",
    ),
    (
        "G. V. Cormack, C. L. A. Clarke, and S. Buettcher, Reciprocal Rank Fusion, SIGIR 2009.",
        "https://research.google/pubs/reciprocal-rank-fusion-outperforms-condorcet-and-individual-rank-learning-methods/",
    ),
    (
        "Sebastian Starke, AI4Animation GitHub repository.",
    ...
    (
        "OpenAI API Docs, GPT-4o mini model page.",
    ...
    (
        "Vite Docs, Getting Started Guide.",
    ...
    (
        "Hardik Pandya, stop-slop GitHub repository.",
        "https://github.com/hardikpandya/stop-slop",
    ),
    (
        "Autodesk, Maya software overview and trial page.",
        "https://www.autodesk.com/products/maya/free-trial",
    ),
    (
        "Blender Foundation, Blender GitHub repository.",
    ...
    (
        "React Three Fiber Docs, Introduction.",
...
def draw_box(
    ...
    outline: tuple[int, int, int] = (43, 52, 61),
) -> None:
    ...
    title_lines = wrap_text(draw, title, font, x2 - x1 - 32)
    body_lines = wrap_text(draw, body, body_font, x2 - x1 - 32)
    y = y1 + 22
    for line in title_lines:
        draw.text((x1 + 16, y), line, font=font, fill=(16, 24, 32))
        y += 32
    y += 4
    for line in body_lines[:5]:
        draw.text((x1 + 16, y), line, font=body_font, fill=(31, 41, 55))
        y += 28
...
def draw_arrow(
    ...
    color: tuple[int, int, int] = (38, 70, 83),
) -> None:
...
def save_diagram(path: Path, title: str, boxes: list[tuple[str, tuple[int, int, int, int], tuple[int, int, int]]], arrows: list[tuple[tuple[int, int], tuple[int, int]]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    img = Image.new("RGB", (1800, 1120), (248, 250, 252))
    draw = ImageDraw.Draw(img)
    draw.rectangle((0, 0, 1800, 92), fill=(17, 24, 39))
    draw.text((42, 25), title, font=get_font(34, bold=True), fill=(255, 255, 255))
    for start, end in arrows:
        draw_arrow(draw, start, end)
    for text, box, fill in boxes:
...
def build_figures() -> dict[str, Path]:
    ...
    figures["architecture"] = FIGURE_DIR / "technical_01_system_architecture.png"
    save_diagram(
        figures["architecture"],
        "Hive Technical System Architecture",
        [
            ("Student kiosk\nReact UI, voice input, avatar state", (70, 160, 420, 330), (219, 234, 254)),
            ("Frontend guard\nGenerated course knowledge and UI answer checks", (70, 450, 420, 620), (224, 242, 254)),
            ("FastAPI backend\n/ask, /health, session history", (560, 160, 910, 330), (220, 252, 231)),
            ("Router and guards\nIntent routing, exact QA lookup, typo aliases", (560, 450, 910, 620), (209, 250, 229)),
            ("Hybrid RAG\nFAISS dense search, BM25, RRF, reranker", (1050, 160, 1400, 330), (254, 243, 199)),
            ("Fine-tuned model\nQwen3.5-2B LoRA adapter in WSL", (1050, 450, 1400, 620), (254, 226, 226)),
            ("Provider layer\nDeepSeek chat mode, OpenAI TTS option, MiniMax comparison", (560, 760, 1400, 930), (237, 233, 254)),
        ],
        [
            ((420, 245), (560, 245)),
            ((735, 330), (735, 450)),
    ...
    figures["fine_tune"] = FIGURE_DIR / "technical_02_finetune_pipeline.png"
    save_diagram(
        figures["fine_tune"],
        "Fine-Tuning Pipeline",
        [
            ("Academic source files\nProgramme structure, prerequisites, course QA", (60, 160, 380, 330), (224, 242, 254)),
            ("QA curation\nRemove duplicates, add source-specific wording", (455, 160, 775, 330), (219, 234, 254)),
            ("Training set\n210 supervised chat examples, v17 JSONL", (850, 160, 1170, 330), (220, 252, 231)),
            ("Unsloth SFT\n4-bit Qwen/Qwen3.5-2B, LoRA r32 alpha64", (1245, 160, 1665, 330), (254, 243, 199)),
            ("Adapter output\nqwen35_2b_lora_out_v17, PEFT LoRA safetensors", (640, 560, 1160, 745), (254, 226, 226)),
            ("Verification loop\nLive QA, RAG eval, UI browser test", (640, 835, 1160, 1010), (237, 233, 254)),
        ],
        [
            ((380, 245), (455, 245)),
            ((775, 245), (850, 245)),
    ...
    figures["rag"] = FIGURE_DIR / "technical_03_hybrid_rag_pipeline.png"
    save_diagram(
        figures["rag"],
        "Hybrid RAG and Accuracy Guards",
        [
            ("Question normalization\nCourse aliases, typo set, Project I/II mapping", (70, 150, 470, 320), (219, 234, 254)),
            ("Exact generated QA\nFast deterministic answer for validated rows", (70, 430, 470, 600), (220, 252, 231)),
            ("Query router\nStructure, details, mixed, clarification", (570, 150, 970, 320), (254, 243, 199)),
            ("Dense retrieval\nEmbeddings and FAISS IndexFlatIP", (1070, 150, 1470, 320), (224, 242, 254)),
            ("Sparse retrieval\nBM25 token search for exact terms", (1070, 430, 1470, 600), (224, 242, 254)),
            ("RRF + rerank\nk=60 fusion and cross-encoder scoring", (570, 710, 970, 880), (254, 226, 226)),
            ("Answer layer\nDirect context answer or DeepSeek generation", (1070, 710, 1470, 880), (237, 233, 254)),
        ],
        [
            ((470, 235), (570, 235)),
            ((270, 320), (270, 430)),
    ...
    figures["avatar"] = FIGURE_DIR / "technical_04_avatar_pipeline.png"
    save_diagram(
        figures["avatar"],
        "Avatar Creation and Motion Pipeline",
        [
            ("Maya source rig\nEbee_Model_rig_New.mb with textures and skeleton", (65, 145, 450, 320), (219, 234, 254)),
            ("Blender and GLB checks\nStatic inspection and mesh validation", (65, 430, 450, 605), (224, 242, 254)),
            ("FBX export\nSkins, smoothing, input connections, no cameras", (560, 145, 945, 320), (220, 252, 231)),
            ("AI4Animation handoff\nUnity exporter, rig map, schema, manifest", (1055, 145, 1440, 320), (254, 243, 199)),
            ("Runtime motion DB\n180 frames, 36 per state, 750 nodePose paths", (1055, 430, 1440, 605), (254, 226, 226)),
            ("Kiosk avatar\nMP4 first, PNG sequence second, pose fallback", (560, 730, 1440, 915), (237, 233, 254)),
        ],
        [
            ((450, 232), (560, 232)),
            ((945, 232), (1055, 232)),
    ...
    figures["accuracy"] = FIGURE_DIR / "technical_05_accuracy_results.png"
    img = Image.new("RGB", (1800, 1120), (248, 250, 252))
    draw = ImageDraw.Draw(img)
    draw.rectangle((0, 0, 1800, 92), fill=(17, 24, 39))
    draw.text((42, 25), "Accuracy Verification Results", font=get_font(34, bold=True), fill=(255, 255, 255))
    rows = [
    ...
    x = 90
    y = 170
    col_widths = [520, 220, 220, 250, 350]
    headers = ["Test set", "Rows", "Passed", "Accuracy", "Path"]
    table_font = get_font(26, bold=True)
    body_font = get_font(25)
    for i, header in enumerate(headers):
        draw.rectangle((x + sum(col_widths[:i]), y, x + sum(col_widths[: i + 1]), y + 62), fill=(38, 70, 83))
        draw.text((x + sum(col_widths[:i]) + 14, y + 17), header, font=table_font, fill=(255, 255, 255))
    y += 62
    for row_index, row in enumerate(rows):
        fill = (255, 255, 255) if row_index % 2 == 0 else (241, 245, 249)
        for i, value in enumerate(row):
            draw.rectangle((x + sum(col_widths[:i]), y, x + sum(col_widths[: i + 1]), y + 68), fill=fill, outline=(203, 213, 225))
            draw.text((x + sum(col_widths[:i]) + 14, y + 18), value, font=body_font, fill=(15, 23, 42))
        y += 68
    metrics = [
        ("UI average response time", "532 ms"),
        ("UI P50 response time", "538 ms"),
        ("UI P95 response time", "791 ms"),
        ("UI max response time", "1155 ms"),
    ]
    base_x = 120
    base_y = 700
    for index, (label, value) in enumerate(metrics):
        bx = base_x + (index % 2) * 760
        by = base_y + (index // 2) * 150
        draw.rounded_rectangle((bx, by, bx + 670, by + 112), radius=14, fill=(220, 252, 231), outline=(22, 101, 52), width=2)
        draw.text((bx + 24, by + 20), label, font=get_font(24), fill=(20, 83, 45))
        draw.text((bx + 24, by + 58), value, font=get_font(34, bold=True), fill=(20, 83, 45))
    img.save(figures["accuracy"])

    figures["provider"] = FIGURE_DIR / "technical_06_provider_tradeoff.png"
    save_diagram(
        figures["provider"],
        "Provider Tradeoff and Runtime Decision",
        [
            ("OpenAI / ChatGPT\nStrong model family and optional OpenAI TTS code path", (80, 170, 520, 350), (219, 234, 254)),
            ("MiniMax\nText, speech, video pricing reviewed for media/provider planning", (80, 520, 520, 700), (224, 242, 254)),
            ("DeepSeek\nOpenAI-compatible chat endpoint with lower verified token price", (680, 170, 1120, 350), (220, 252, 231)),
            ("Local Qwen runtime\nWSL fine-tuned backend powers verified /ask accuracy", (680, 520, 1120, 700), (254, 243, 199)),
            ("Final policy\nUse RAG and local model for accuracy; keep DeepSeek as cloud LLM option", (1280, 330, 1710, 540), (237, 233, 254)),
        ],
        [
            ((520, 260), (680, 260)),
            ((520, 610), (680, 610)),
    ...
    figures["ui_landscape"] = SOURCE_FIGURE_DIR / "figure_01_ui-refresh-landscape.png"
    figures["ui_portrait"] = SOURCE_FIGURE_DIR / "figure_02_ui-refresh-portrait.png"
    figures["avatar_video_verified"] = SOURCE_FIGURE_DIR / "figure_03_avatar-video-only-verified.png"
    figures["avatar_contact_sheet"] = SOURCE_FIGURE_DIR / "figure_18_avatar_state_contact_sheet.png"
...
def add_paragraph(doc: Document, text: str = "", style: str | None = None, align: int | None = None) -> None:
...
def source_facts() -> dict:
    summary = read_json(PACK / "02_project_evidence" / "evaluation_summary.json")
    live_summary = read_text(ROOT / "hybride serch master file" / "reports" / "live_eval_accuracy_summary.md")
    return {
        "qa_all": summary.get("qa_all", {}),
...
def section_texts() -> list[tuple[str, list[str]]]:
    return [
        (
...
def important_snippets() -> list[tuple[str, str]]:
    return [
        (
...
def technical_tables() -> dict[str, list[tuple[list[str], list[list[str]]]]]:
    return {
        "3.1 Fine-Tuning Data and Training Pipeline": [
            (
        ...
        "3.4 Avatar Creation and Motion Design": [
            (
                ["Layer", "Artifact", "Role"],
                [
                    ["Maya source", r"C:\Users\jeysa\Desktop\Ebee_New\Ebee_Model_rig_New.mb", "Authoritative rigged model"],
...
def build_markdown(figures: dict[str, Path], facts: dict) -> str:
    lines: list[str] = []
    ...
    lines.append("")
    for title, paragraphs in section_texts():
        level = "##" if title.startswith("CHAPTER") else "###"
        lines.append(f"{level} {title}")
        lines.append("")
        for paragraph in paragraphs:
            lines.append(paragraph)
            lines.append("")
        for headers, rows in technical_tables().get(title, []):
            lines.append("| " + " | ".join(headers) + " |")
            lines.append("| " + " | ".join(["---"] * len(headers)) + " |")
            for row in rows:
                lines.append("| " + " | ".join(row) + " |")
            lines.append("")
        if title == "CHAPTER 3: DETAILS OF THE DESIGN":
            for key, caption in [
                ("architecture", "Figure 3.1: Technical system architecture."),
                ("fine_tune", "Figure 3.2: Fine-tuning pipeline."),
                ("rag", "Figure 3.3: Hybrid RAG pipeline."),
                ("avatar", "Figure 3.4: Avatar creation and motion pipeline."),
                ("provider", "Figure 3.5: Provider tradeoff and runtime decision."),
            ]:
                lines.append(f"![{caption}]({figures[key]})")
                lines.append("")
        if title == "3.11 UI/UX Implementation and Visual Evidence":
            for key, caption in [
                ("ui_landscape", "Figure 3.6: Landscape Hive UI with Ebee avatar and advising chat."),
                ("ui_portrait", "Figure 3.7: Portrait Hive UI showing responsive kiosk layout."),
                ("avatar_video_verified", "Figure 3.8: Verified generated-video Ebee avatar in the Hive UI."),
                ("avatar_contact_sheet", "Figure 3.9: Avatar state contact sheet for idle, listening, thinking, speaking, and review states."),
            ]:
                lines.append(f"![{caption}]({figures[key]})")
                lines.append("")
        if title == "CHAPTER 4: DATA PRESENTATION AND DISCUSSION OF FINDINGS":
            lines.append(f"![Figure 4.1: Accuracy verification results.]({figures['accuracy']})")
            lines.append("")
    lines.append("## Selected Important Code Snippets")
    for title, snippet in important_snippets():
    ...
    for i, (label, url) in enumerate(REFERENCE_LIST, start=1):
        lines.append(f"[{i}] {label} Available: {url}. Accessed: {ACCESS_DATE}.")
    lines.append("")
    lines.append("## Appendix A: Evidence Files")
    lines.extend(
        [
...
def build_docx(figures: dict[str, Path], facts: dict) -> Path:
    doc = Document()
    configure_document(doc)

    add_paragraph(doc, REPORT_TITLE, align=WD_ALIGN_PARAGRAPH.CENTER)
    add_paragraph(doc, f"By\n{AUTHOR}\n{STUDENT_ID}", align=WD_ALIGN_PARAGRAPH.CENTER)
    add_paragraph(
        doc,
        f"The project report is prepared for\n{FACULTY},\n{UNIVERSITY}\nin partial fulfilment of\nBachelor of Science (Honours) in Intelligent Robotics\n\n{SESSION}",
    ...
    add_paragraph(doc, f"Signature: ________________________\nName: {AUTHOR}\nStudent ID: {STUDENT_ID}\nDate: ________________________")
    doc.add_page_break()

    add_heading(doc, "ACKNOWLEDGEMENT", 1)
    add_paragraph(
        doc,
        "I would like to thank the project supervisor for guidance on the system direction, evaluation scope, and report requirements. I also thank family, friends, and testers who gave feedback during the kiosk, RAG, and avatar verification stages.",
    )
    doc.add_page_break()
    ...
    add_paragraph(
        doc,
        "Hive is an academic advising kiosk for Intelligent Robotics students. The system combines a fine-tuned Qwen/Qwen3.5-2B LoRA adapter trained with Unsloth, hybrid dense-sparse retrieval, deterministic programme guards, provider-aware LLM integration, browser voice input, speech output, and an Ebee avatar. The verified local runtime uses a WSL fine-tuned Qwen backend for the tested /ask path, while the Windows backend keeps DeepSeek as an OpenAI-compatible cloud generation option. Retrieval combines FAISS dense search, BM25 sparse search, Reciprocal Rank Fusion, and optional cross-encoder reranking. Accuracy improved through duplicate QA removal, exact generated-QA lookup, rebuilt RAG indexes, frontend guard fixes, and browser UI testing. Final evaluation recorded 1500/1500 mixed API passes and 300/300 rendered-UI passes with 532 ms average UI response time. The production avatar uses generated Ebee video and PNG state assets, while the standalone showcase keeps the full Maya/FBX, Blender/GLB inspection, Unity AI4Animation, and Three.js rigged-avatar path.",
    )
    doc.add_page_break()
    ...
    toc_rows = [
        ...
        ["Chapter 4", "Data Presentation and Discussion of Findings"],
        ["Chapter 5", "Conclusions and Recommendations"],
        ["Appendix A", "Evidence Files and Commands"],
    ]
    add_table(doc, ["Section", "Title"], toc_rows)
    doc.add_page_break()
    ...
    figure_map = {
        "CHAPTER 3: DETAILS OF THE DESIGN": [
            ("architecture", "Figure 3.1: Technical system architecture."),
            ("fine_tune", "Figure 3.2: Fine-tuning pipeline."),
            ("rag", "Figure 3.3: Hybrid RAG and accuracy guards."),
            ("avatar", "Figure 3.4: Avatar creation and motion pipeline."),
            ("provider", "Figure 3.5: Provider tradeoff and runtime decision."),
        ],
        "CHAPTER 4: DATA PRESENTATION AND DISCUSSION OF FINDINGS": [
            ("accuracy", "Figure 4.1: Accuracy verification results."),
        ],
        "3.11 UI/UX Implementation and Visual Evidence": [
            ("ui_landscape", "Figure 3.6: Landscape Hive UI with Ebee avatar and advising chat."),
            ("ui_portrait", "Figure 3.7: Portrait Hive UI showing responsive kiosk layout."),
    ...
    for title, paragraphs in section_texts():
        ...
        for key, caption in figure_map.get(title, []):
            add_figure(doc, figures[key], caption)

    doc.add_page_break()
    add_heading(doc, "Selected Important Code Snippets", 1)
    add_paragraph(
        doc,
        "This appendix keeps code to the implementation points needed to explain the design. The full source files remain in the project workspace and report pack.",
    )
    for title, snippet in important_snippets():
        add_heading(doc, title, 2)
        code_block(doc, snippet)

    doc.add_page_break()
    add_heading(doc, "REFERENCES", 1)
    for i, (label, url) in enumerate(REFERENCE_LIST, start=1):
        add_paragraph(doc, f"[{i}] {label} Available: {url}. Accessed: {ACCESS_DATE}.")

    doc.add_page_break()
    add_heading(doc, "APPENDIX A: EVIDENCE FILES AND COMMANDS", 1)
    evidence_rows = [
        ["Fine-tune workspace", r"Ubuntu-24.04:/home/jet/fyp-unsloth"],
        ["Training script", r"/home/jet/fyp-unsloth/scripts/train_qwen35_unsloth.py"],
        ["Adapter output", r"/home/jet/fyp-unsloth/outputs/qwen35_2b_lora_out_v17"],
        ["Live accuracy summary", str(ROOT / "hybride serch master file" / "reports" / "live_eval_accuracy_summary.md")],
        ["RAG status", str(ROOT / "frontend" / "docs" / "RAG_ACCURACY_STATUS.md")],
        ["Exact avatar guide", str(ROOT / "frontend" / "public" / "avatar" / "exact" / "EXPORT_GUIDE.md")],
        ["AI4Animation handoff", str(ROOT / "frontend" / "showcase" / "animated-avatar" / "docs" / "AVATAR_AI4ANIMATION_HANDOFF.md")],
    ]
    add_table(doc, ["Evidence", "Path"], evidence_rows)
    add_heading(doc, "Verification Commands", 2)
    code_block(
        doc,
...
npm run kiosk:check""",
    ...
    try:
        ...
        return REPORT_DOCX_FALLBACK


def run_static_checks(markdown: str) -> dict:
    banned = [
        "Here's",
        "here's",
    ...
    hits = [item for item in banned if item in markdown]
    return {
        "banned_pattern_hits": hits,
        "em_dash_count": markdown.count("—"),
        "en_dash_count": markdown.count("–"),
    }
...
def main() -> None:
    ...
    facts = source_facts()
    markdown = build_markdown(figures, facts)
    checks = run_static_checks(markdown)
    REPORT_MD.write_text(markdown, encoding="utf-8")
    docx_path = build_docx(figures, facts)

    summary = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "docx": str(docx_path),
        "canonical_docx": str(REPORT_DOCX),
        "markdown": str(REPORT_MD),
        "figures": {key: str(value) for key, value in figures.items()},
        "static_checks": checks,
        "external_sources_accessed": [url for _, url in REFERENCE_LIST],
        "notes": [
            "No API key values are included in the generated report.",
            "MiniMax is documented as a provider comparison path because no MiniMax production client was found in the searched workspace.",
            "The verified browser and API accuracy path used the local WSL fine-tuned backend.",
        ],
    }
